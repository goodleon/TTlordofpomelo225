		...
		for(var i = 0; i < data[key][ke].length; i++){
			...
			for(var j = 0; j < obje.length; j++){
				...
				if(obje["clientPortStart"]){
					ob.push({"restart-force":true,"auto-restart":true,"id": id, "host": obje["host"], "port": obje["portStart"]+j, "clientPort": obje["clientPortStart"]+j, "frontend": true});
				}
				else{
					ob.push({"restart-force":true,"auto-restart":true,"id": id, "host": obje["host"], "port": obje["portStart"]+j});
				}
			}
		}
