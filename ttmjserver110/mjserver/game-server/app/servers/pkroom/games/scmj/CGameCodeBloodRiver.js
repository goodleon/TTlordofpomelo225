//**************************************
//血流成河---玩法-血流成河---CGameCodeBloodRiver
//create by huyan
//
/**
 * 拍桌状态
...
function CGameCodeBloodRiver(majiang, _GLOG, _app, tTable, logid, gameid, publicIp) {
    this.majiang = majiang;
    this.GLog = _GLOG;
    this.app = _app;
    this.GLog("CGameCodeBloodRiver");
    this.tTable = tTable;
    this.existCheck = {};
    this.logid = logid;
    this.gameid = gameid;
...
g_Inherit(require("./CGameCodeFightEnd.js"), CGameCodeBloodRiver)


//发牌不要求在线
CGameCodeBloodRiver.prototype.SendNewCard = function(tb, from) {
    var tData = tb.tData;
    var cards = tb.cards;
    ...
    if (tb.AllPlayerCheck(function(pl) {
        ...
        if (tData.cardNext < cards.length) {
            ...
            this.GLog("CGameCodeBloodRiver------------>newCard");
            return true;
        } else //没有牌了 
        {
...
CGameCodeBloodRiver.prototype.EndGame = function(tb, pl, byEndRoom) {
    var tData = tb.tData;
    var fakeWin = [];
    var fakeLose = [];
    var gThis = this;
    ...
    this.GLog("CGameCodeBloodRiver------------>roundEnd");
    var playInfo = null;
    if (tData.roundNum == 0) {
...
CGameCodeBloodRiver.prototype.EndCalBloodWinOne = function(tb) {
    var tData = tb.tData;
 ...
 * 杠 
 */
CGameCodeBloodRiver.prototype.MJGang = function(pl, msg, session, next, pTable) {
    ...
    if (pl.mjhu.length == 0) { // 1: 没有胡牌 可以扛牌
        isBlood = true;
    }
    ...
    if (!isBlood) {
        this.GLog("BloodRiver :Table.prototype.MJGang blood return");
        return;
    }
...

    {
        ...
        this.GLog("CGameCodeBloodRiver------------>MJGang");
 ...
 * 过(什么都不干) tag fix
 */
CGameCodeBloodRiver.prototype.MJPass = function(pl, msg, session, next, tTable) {
        ...
        if (tData.tState == TableState.waitEat && pl.mjState == TableState.waitEat && pl.winType <= 0) {
            ...
            if (pl.eatFlag == msg.eatFlag && tTable.CheckPlayerCount(function(p) {
                ...
                this.GLog("CGameCodeBloodRiver------------>MJPass");
            ...
            this.GLog("CGameCodeBloodRiver .MJPass " + " pl.mjState = TableState.isReady");
     ...
     * 碰牌
     */
CGameCodeBloodRiver.prototype.MJPeng = function(pl, msg, session, next, tTable) {
    var isAutoPut = true;
    if (next) {
        next(null, null); //if(this.GamePause()) return;
        isAutoPut = false;
    ...
    if (tData.tState == TableState.waitEat &&
        ...
        if (tTable.AllPlayerCheck(function(p) { if (p == pl) return true; return p.eatFlag < 8; })) {
            ...
            if (!isAutoPut) {
                this.GLog("CGameCodeBloodRiver------------>MJPeng call InitAutoState");
            ...
            if (matchnum >= 2) {
                ...
                this.GLog("CGameCodeBloodRiver------------>MJPeng");
            } else {
 ...
 * 血流成河胡牌
 */
CGameCodeBloodRiver.prototype.MJHu = function(pl, msg, session, next, isFromPass, tTable) {
    //此处必须保证胡牌顺序
    var tData = tTable.tData;
    var uids = tTable.tData.uids;
    var canEnd = false;

    if (!tData.noBigWin && this.majiang.cardTypeNum(pl.mjhand, pl.mjMiss) > 0) {
        return;
    }
    this.GLog("MJHuBlood:2!");
    //已经胡
    //if(pl.winType>0) return;
    var mjHuMsg = { wins: {} };
    var huPl = [];
    //自摸胡
    if (
    ...
    //点炮胡 抢杠胡
        ...
        if (tData.tState == TableState.waitEat) {
            ...
            if (tData.putType == 0) {
                ...
                fp.mjdesc.push("点炮");
                //杠上炮 退回所有杠
                //ReturnGangWin(this,fp,fp.lastGang,false);
            ...
            this.GLog("huyan:BloodReturnGangWin tData.putType" + tData.putType);
    ...
    if (canEnd) {
        ...
        if (tData.winner == -1) {
            ...
            else tData.winner = tData.curPlayer;
        }
        //计算番
        for (var i = 0; i < huPl.length; i++) {
            var pi = huPl[i];
            if (tData.wuid.indexOf(pi.uid) < 0) {
                tData.wuid.push(pi.uid);
            }
            pi.lastCard = pi.mjhand[pi.mjhand.length - 1];
            mjHuMsg.huCard = pi.lastCard;
            this.GLog("pi--------------1");
            if (pi.winType >= WinType.pickNormal) { //自摸
                this.GLog("pi--------------2");
                pi.mjhand.splice(pi.mjhand.length - 1, 1);
                pi.mjhu.push(pi.lastCard);
            } else {
                this.GLog("pi--------------3");
                pi.mjhand.splice(pi.mjhand.length - 1, 1);
                pi.mjhu.push(pi.lastCard);
            }
            mjHuMsg.wins[pi.uid] = { lastCard: pi.lastCard, winType: pi.winType, mjhu: pi.mjhu.slice(0) };

            var baseWin = 1;

            if (!tData.noBigWin) {
                this.GLog("111111111111");
                this.GLog(pi.mjdesc);
                this.GLog(pi.mjhand);
                //处理胡牌类型，将胡牌加到mjhand里。计算类型后再删除
                pi.mjhand.push(pi.mjhu[pi.mjhu.length - 1]);
                var genArr = [];
                baseWin = this.majiang.computeBaseWin(pi, true, tData, false, genArr);
                pi.mjhand.splice(pi.mjhand.length - 1, 1);


                var firstWin = pi.winType == WinType.pickNormal && tData.cardNext == 53; //天胡
                var secondWin = pi.winType == WinType.pickNormal && pi.getNum == 1 && tData.cardNext != 53; //地胡
            ...
            if (baseWin == 1)
                pi.mjdesc.push("平胡");
            else if (genArr.length > 0) { //处理根的影响
        ...
        if (
            tTable.CheckPlayerCount(function(pj) {
                if (pj.winType <= 0) {
                        ...
                        if (tData.gshdianpao) {
                            if (pi.winType == WinType.pickGang1) {
                                if (pi.lastGangPlayer >= 0 && pj.uid == tData.uids[pi.lastGangPlayer]) {
                                    ...
                                    // pj.winone -= (roundWin * pi.baseWin); //del by wcx 20170120
                                    //add by wcx 20170120
                    ...
                    if (!tData.noBigWin) pj.mjState = TableState.waitCard;
                    return true;
                } else {
                    if (!tData.noBigWin) pj.mjState = TableState.waitCard;
                    return true;
            ...
            this.GLog("222222---------22222");
            mjHuMsg.winone = tTable.PlayerPtys(function(p) { return p.winone; });
            tTable.AllPlayerRun(function(p) {
                p.winType = 0;
                tData.huResult.push({ "uid": p.uid, "winone": p.winone }); //记录胡牌信息
            });
            tTable.mjlog.push("MJHu", mjHuMsg);
            this.GLog("CGameCodeBloodRiver------------>MJHu 1");
            //总结算
            this.EndCalBloodWinOne(tTable);
            this.EndGame(tTable, pl);
        } else {
            mjHuMsg.winone = tTable.PlayerPtys(function(p) { return p.winone; });
            this.GLog("mjHuMsg.winone");
            var duoPaoNum = 0;

            var beforeWinIndex = tData.uids.indexOf(pl.uid);
            for (var i = 0; i < 4; i++) {
                var beforeIndex = (tData.curPlayer - 1 - i + tData.maxPlayers) % tData.maxPlayers;
                var bpl = tTable.getPlayer(tData.uids[beforeIndex]);
                if (bpl.winType > 0) {
                    beforeWinIndex = beforeIndex;
                    break;
                }
            }
            tTable.AllPlayerRun(function(p) {
                ...
                p.winType = 0;
                tData.huResult.push({ "uid": p.uid, "winone": p.winone, "desc": p.mjdesc }); //记录胡牌信息
            });
            this.GLog("tData.huResult------->");
            this.GLog(tData.huResult);
            tTable.newNotifyAllWithMsgID("MJHu", mjHuMsg);
            tTable.mjlog.push("MJHu", mjHuMsg);
            this.GLog("CGameCodeBloodRiver------------>MJHu 2");
            //对于一炮多响，需要修正胡的玩家
            if (tData.blood && duoPaoNum > 1) {
                tData.curPlayer = beforeWinIndex;
            } else {
                tData.curPlayer = tData.uids.indexOf(pl.uid);
            }
            tData.putType = 0;
...
module.exports = CGameCodeBloodRiver;