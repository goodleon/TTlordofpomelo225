module.exports = function(app, server, gameid, Player, Table, TableGroup, TableManager, Game) {
    ...
    function GLog(log) { app.FileWork(gameLog, "/root/mjserver/log.txt", log) }
    ...
    Table.prototype.SetTimer = function() {
        ...
        if (delayTimer.timerId) { clearTimeout(delayTimer.timerId);
    ...
    Table.prototype.FullStartCheck = function() {
        ...
        if (this.IsFull()) {
            ...
            tb.SetTimer(
                1000,
                function() { if (tb.IsFull()) tb.willStart(); }, 4000,
                function() { if (tb.IsFull()) tb.startGame(); }
            );
        } else if (this.PlayerCount() == 1) //第一个玩家开启npc timer
        {
            ...
            tb.SetTimer(
                ...
                function() { tb.willStart() }, 4000,
                function() { if (tb.PlayerCount() > 0) tb.startGame(); }
