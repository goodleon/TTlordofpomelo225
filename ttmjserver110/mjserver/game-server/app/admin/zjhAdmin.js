...
function GLog(app, log) { app.FileWork(gameLog, __dirname + "/log.txt", log) }
...
var serverHandler = {
    ...
    pkplayerLogout: function(agent, msg, next) {
        ...
        if (pl) {
            ...
            var server = app.GetServerBuyUid("pkplayer", uid);
            httpClient.postJson("forceLogout", { uid: uid },
                server.port + 1000, server.host, cb);
        }
    ...
    vipTableList: function(agent, msg, next) {
        ...
        if (msg.tableid) {
            var rtn = {};
            rtn[msg.tableid] = vipTable[msg.tableid];
            next(null, rtn);
    ...
    doLogout: function(agent, msg, cb) {
        if (this.app.auth && this.app.serverType == 'auth')
            this.app.auth.doLogout(false, msg.uid, msg.sid, msg.fid, {}, false,
                function(e, r) {
                    if (cb) cb(e, r);
                });
    },
    removeServer: function(agent, msg, cb) {
        ...
        if (canStop) exec('pomelo stop  -u admin -p 1qaz2wsx3edc -P ' +
            this.app.getMaster().port + ' ' + this.app.serverId,
            function(err, rtn) {
                cb(err, rtn);
            });
...
Module.prototype.monitorHandler = function(agent, msg, cb) {
    if (!msg) //定时报告
    {
        ...
        var report = {
            serverId: agent.id,
            sqlinfo: this.app.sqlinfo,
            sqlok: this.app.sqlok,
            sqlerr: this.app.sqlerr,
            rpcinfo: rpcinfo
        };
        for (var k in report.sqlinfo) {
            var n0 = report.sqlinfo[k];
            var n1 = report.sqlok[k];
            if (!n1) n1 = 0;
            var n2 = report.sqlerr[k];
            if (!n2) n2 = 0;
            ...
            case 'rooms':
                {
                    ...
                    function AddToReturn(mgr) {
                        ...
                        less.push('less');
                        mgr.less.forEach(function(id) {
                            var rd = mgr.all[id].round;
                            less.push(id + "_" + rd.joinNum + "_" + rd.watchNum);
                        });
                        less.push('empty');
                        mgr.empty.forEach(function(id) {
                            var rd = mgr.all[id].round;
                            less.push(id + "_" + rd.joinNum + "_" + rd.watchNum);
                        });
                        less.push('full');
...
var clientHandler = {
    getDayData: function(agent, msg, cb) {
        ...
        var dbPlay = db;
        if (app.mdb && app.mdb.db) dbPlay = app.mdb.db;
