...
domain.on('error', function(err) {
    ...
    var serverid = "";
    if (domain.pomeloApp) {
        serverid = domain.pomeloApp.serverId;
    }

    fs.appendFileSync('uncaught/' + serverid +
        "_" + (new Date().Format("yyyy_MM_dd")) +
        ".txt", '\ndomain ' +
        err.stack);
...
domain.run(function() {
    ...
    app.configure( //'production|development',
        function() {
            ...
            function DirectRoute(serverid, msg, app, cb) {
                cb(null, serverid);
            }
            ...
            function SessionRoute(sType) {
                return function(session, msg, app, cb) {
                    ...
                    if (!serverid && app.serverType == "pkcon" &&
        ...
        if (app.serverType != 'master') {
            app.components.__proxy__.client._station.on('error',
                function(code, tracer, serverId, msg, opts, cb) {
                    app.FileWork(rpcErr, "uncaught/rpcErr_" +
                        app.serverId + ".txt", code + ' ' +
                        serverId + " " +
                        JSON.stringify(msg))
                });
        } else {}
    ...
    process.on('uncaughtException', function(err) {
        fs.appendFileSync('uncaught/' +
            app.serverId + "_" + (new Date().Format("yyyy_MM_dd")) + ".txt", '\nuncaught ' + err.stack);
