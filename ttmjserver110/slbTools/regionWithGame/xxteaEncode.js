...
var toPath = process.argv[3] || null;
function writeEncode( filepath, buff, to )
{
    filepath = filepath.replace( ".json", ".dat" );
    fs.writeFileSync(filepath,buff, "utf-8");
	console.log("write new file " + filepath);

	if(to)
    {
	    cp.execSync('cp '+ filepath +' ' + to + '&&rm '+ filepath);
	    console.log( "copy " + filepath + " to " +  to + '&&rm '+ filepath);
    }
}

function _main( )
{
     console.log( originPath, toPath );
	...
	fs.readdir(originPath, function(err, files){
		...
		files.forEach(function (file)
		{
			...
			var jsonFile = require(originPath + file);
			if(jsonFile.servers)
			{
				var buff = fs.readFileSync( originPath + file, "utf-8");
				buff = encodeURI( buff );
				buff = xxtea_encrypt( buff, "pipi2016" );
				var base64_1 = base64.base64encode( buff );
				writeEncode(originPath + file, "encode" + base64_1, toPath != toPath? toPath : null);
			}
		});
